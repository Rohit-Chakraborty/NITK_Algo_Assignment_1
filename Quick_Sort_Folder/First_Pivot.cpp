...
// Function to partition the array on the basis of the first element as pivot
int partition(std::vector<int>& arr, int low, int high) {
    ...
    std::swap(arr[i - 1], arr[low]); // Place the pivot element at its correct position
    return (i - 1);
}

// Function to perform quicksort using the first element as the pivot
void quickSortFirstPivot(std::vector<int>& arr, int low, int high) {
    if (low < high) {
...
int main() {
    ...
    std::string timingFile = "C:/Users/91907/OneDrive/Desktop/Algo Assgn 1/Outputs/Timing/Quick_Sort.txt";
    ...
    for (const auto& filename : files) {
        ...
        // Perform quicksort with the first element as the pivot
        quickSortFirstPivot(data, 0, data.size() - 1);
